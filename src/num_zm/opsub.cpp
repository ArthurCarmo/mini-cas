/*
 * Subtraction operators for the modulo arithmetic integers class
 *
...
num_zm & num_zm::operator-=(const num_zm &a){
	if(this->_base == a._base){	
		this->_num -= a._num;
		if(this->_num._sign != this->_base._sign && !this->_num.is_zero())
			this->_num = num_z(this->_base) + this->_num;
