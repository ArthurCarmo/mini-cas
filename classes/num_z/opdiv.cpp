...
div_tuple num_z::operator/(const num_z &a){ 
	div_tuple res;	
	num_z m(*this, this->_blocks + 1), n(a), parc_m(0, this->_blocks + 1), parc_n(a, a._blocks);
	int64_t i, j;
	...
	if(n._blocks == 1){
		...
		res.r._sign = res.r._num[0]==0?0:a._sign;
		return res;
	}
	...
	for(j = m._blocks - 1; j > n_size; --j){	
		...
		parc_m._blocks = n._blocks + 1;
		for(i = 0; i < (n._blocks+1); i++){
